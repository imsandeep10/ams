...
import { useGetAdminById, useUpdateAdmin } from "@/lib/api/useAdmin";
...
function EditAdminFormComponent() {
  ...
  const { data: adminData, isLoading, error } = useGetAdminById(id || "");
  ...
  useEffect(() => {
    if (adminData?.data && !isLoading && !isFormInitialized) {
      form.reset({
  ...
  }, [adminData, isLoading, isFormInitialized, form]);
  ...
  const onSubmit = useCallback(
    async (values: EditAdminFormData) => {
      if (!id) return;

      try {
        await updateAdmin({ id, data: values });
          ...
          <Form {...form}>
            ...
            >
              ...
              <div className="space-y-4">
                ...
                {(previewImage || adminData.data.profileImage) && (
              ...
              <div className="pt-6 border-t flex gap-3">
                <Button
                  type="submit"
                  className="px-8 py-3 font-medium transition-all duration-200 hover:shadow-lg disabled:opacity-50"
...
export const EditAdminForm = memo(EditAdminFormComponent);
