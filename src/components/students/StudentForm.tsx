...
function CreateStudentFormComponent({ mode }: props) {
    ...
    async (values: CreateStudentFormData) => {
      try {
        ...
        if (mode === "edit" && id) {
          ...
          res = await updateStudent({ id, data: values });
        } else {
          // Create new student
          res = await createStudents(values);
        }
      ...
      <Form {...form}>
        ...
        >
          ...
          <div className="md:col-span-2">
            <Button
              type="submit"
              className="w-full cursor-pointer py-3 font-medium transition-all duration-200 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={
...
export const CreateStudentForm = memo(CreateStudentFormComponent);
